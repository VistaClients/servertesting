...
function uploadImgData(file, originalEl) {
    ...
    $.ajax({
        ...
        contentType: false,
        success: function () {
            // ONLY preview path changes
            const previewPath = `/static/${clientName}/${clientProjectName}/${originalSrc}`;
            $(originalEl).attr('src', previewPath);
        },
        error: function (xhr) {
...
function openImagePicker(targetEl) {
    ...
    if (selectedFile) {
        const relativePath = `assets/images/${selectedFile.name}`;
        $(el).attr('data-original-src', relativePath);
        uploadImgData(selectedFile, el);

    } else if (selectedImageSrc) {
        const filename = selectedImageSrc.split('/').pop().split('?')[0];
